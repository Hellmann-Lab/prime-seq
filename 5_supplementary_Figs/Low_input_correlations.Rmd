---
title: "Low_input_comparability"
output: html_notebook
---

Goal: Show that 1k Cells and 10k Cells yield similar data

Plan: Do DESeq2 normalisation and plot sample distances as well as PCA and gene wise correlations.
...
###2. Load following functions, theme and color palets:
...
data_path<-"/data/share/htp/prime-seq_Paper/Fig_beads_columns/"
...
counts <- readRDS(paste0(data_path,"zUMIs/HEK/zUMIs_output/expression/Bulk_opt_lysis_test_2_HEK.dgecounts.rds"))

inf <- read.csv(paste0(data_path,"analysis/Lysis/sample_info.csv"), header = T, stringsAsFactors = F)
...
inex_counts<-counts$umicount$inex$all[,inf$BC]%>% as.matrix()

inf<-inf[match(colnames(inex_counts),inf$BC),]

```

###4. create filter and log normalize
...
sampleCorrelation <- cor(inex_counts_filt_log,method = "pearson")^2
...
sample_cor<-ggplot(cor_df,aes(x=input_col,y=cor,colour=input_col))+
  ...
  geom_label(data=mean_df,aes(label=cor,y=0.93,fill=input_col),colour="white")+
...
gene_cor<-ggplot(data=subset(mean_exp,spike_in=="Endogenous Genes"),aes(x=`high input`,y=`low input`))+
  ggpubr::stat_cor(data=subset(mean_exp,spike_in=="Endogenous Genes"),aes(fill=spike_in, label=paste(..rr.label..)),
                   ...
                   geom = "label",
                   colour="white")+
  stat_binhex(binwidth=c(0.1,0.1),aes(fill=stat(log(count))))+
